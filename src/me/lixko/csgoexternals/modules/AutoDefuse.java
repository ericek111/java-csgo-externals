...
public class AutoDefuse extends Module {
	...
	private Thread autodefuse = new Thread(new Runnable() {
		...
		public void run() {
			...
			while(true) {
				...
				if(!autodefuset || Offsets.m_dwLocalPlayer == 0) continue;
				...
				if(bombtimer < 0f || defused) {
					if(team == 3) Engine.clientModule().writeInt(Offsets.input.use, 4);
					bombentityaddr = 0;
					autodefuset = false;
					hastimetodefuse = false;
				}
	...
	public void onUIRender() {
		...
		if(!autodefuset || bombtimer < 0) return;
		...
		if(hastimetodefuse) {
			...
			else DrawUtils.setTextureColor(1f, 0f, 0f);
