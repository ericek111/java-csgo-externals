...
public class RankReveal extends Module {
	...
	private Map<Integer, Integer> tbs_scoreboardPlayersT = new HashMap<Integer, Integer>(); // to be sorted
	private Map<Integer, Integer> tbs_scoreboardPlayersCT = new HashMap<Integer, Integer>(); // to be sorted
		...
		public void run() {
			while (Client.theClient.isRunning) {
				try {
					...
					if(Offsets.m_dwPlayerResourcesPointer == 0) continue;
					if(Offsets.m_dwPlayerResources == 0) continue;
					...
					for(int i = 0; i < 63; i++) {
						if(m_bConnected[i] == 0) continue;
						if(m_iTeam[i] == 2) tbs_scoreboardPlayersT.put(i, m_iScore[i]);
						if(m_iTeam[i] == 3) tbs_scoreboardPlayersCT.put(i, m_iScore[i]);
					}
	...
	public void onUIRender() {
        ...
        for (Map.Entry<Integer, Integer> entry : scoreboardPlayersCT.entrySet()) {
			drawEntity(entry.getKey(), DrawUtils.getHeight() - ctyoffset - iter * SCOREBOARD_PLAYER_HEIGHT);
			iter++;
    	}
        ...
        for (Map.Entry<Integer, Integer> entry : scoreboardPlayersT.entrySet()) {
			drawEntity(entry.getKey(), DrawUtils.getHeight() - tyoffset - iter * SCOREBOARD_PLAYER_HEIGHT);
			iter++;
    	}
	...
	private void drawEntity(int resid, int y) {
		DrawUtils.drawString(DrawUtils.getWidth()/4*3, y,  resid + "");
	}
